...
# 5 fold validation for each gene
# read from h5py file
...
if __name__ == "__main__":
    ...
    existing_genes = ["AASS", "AATF", "ABL1", "ABRA", "ABT1",
                      "ACTB", "ACTL6A", "ACTL6B", "ACTN1", "ACTN2",
                      "ACTN4", "ACTR5", "ACTR6", "ACTR8"]
    ...
    for specific_gene in sorted(list(unique_genes)):
        ...
        if specific_gene in existing_genes: continue
        ...
        #train only if datapoints more than 633 (90% percentile, used to be 100)
        train_fold_results = []
        valid_fold_results = []
        if len(training_labels) < 633:
