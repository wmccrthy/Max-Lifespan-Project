...
# using profiler to figure out blocks
...
class EnformerDataModule(L.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, num_workers=0, drop_last = True)
    
    def val_dataloader(self):
        return DataLoader(self.valid_dataset, batch_size=self.batch_size, num_workers=0, drop_last = True)

if __name__ == "__main__":
    batch_size, num_epochs, num_dev = int(2), int(2), int(1)
    max_data = 1000
    pr = cProfile.Profile()
    pr.enable()
    ...
    with open('profile_output.txt', 'w') as f:
        ...
        ps = pstats.Stats(pr, stream=f).sort_stats(sortby)  # Redirect output to file
        ps.print_stats()  # This will write to the file